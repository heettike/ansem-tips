...
const settingsSchema = z.object({
  ...
  commentTrigger: z.string().trim().min(1).max(50).optional(),
  superTipTrigger: z.string().trim().min(1).max(50).optional(),
  tipChain: z.enum(["solana", "base", "bsc", "robinhood"]).optional(),
  tipTokenAddress: z.string().trim().max(64).optional(),
  tipTokenSymbol: z.string().trim().min(1).max(20).optional(),
  tipTokenDecimals: z.number().int().min(0).max(18).optional(),
  enabled: z.boolean(),
});
